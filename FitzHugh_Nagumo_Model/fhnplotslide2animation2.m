...
i = 0.05;
%i = 0.04;
[t,y] = ode45(@(t,y) fhn(t, y, alpha, gamma, epsilon, i), tspan, y0);%plotting
%colors:
...
curve = animatedline('Color', red_color, 'LineWidth', 2.5);

xlim([0 400]) 
ylim([-0.5 1])

video = VideoWriter('video2c.avi', 'MPEG-4');
open(video)

for i = 1:length(y(:,1))
    addpoints(curve, t(i), y2(i));
    head = scatter(t(i), y2(i), 'filled', 'MarkerFaceColor', background_tan_color, 'MarkerEdgeColor', red_color);
    drawnow
    pause(0.0001)
